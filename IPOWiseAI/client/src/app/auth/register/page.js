...
export default function Register() {
    ...
    const googleSignIn = async () => {
        const { data, error } = await supabase.auth.signInWithOAuth({
            provider: 'google',
            options: {
                redirectTo: 'http://localhost:3000/auth/register/user-info'
            }
        })
    ...
    const OnSubmitHandler = async (e) => {
        ...
        if (emailRegex.test(email) && password.length > 0) {
            ...
            const { data, error } = await supabase.auth.signUp({
                ...
                password,
                options: {
                    emailRedirectTo: 'http://localhost:3000/auth/register/user-info'
                }
            })
            console.log(data, error)
            if (!error) {
            ...
            <div classNameName="max-w-7xl mx-auto px-6 md:px-12 xl:px-6">
                <div className="relative">
                    <div className="max-w-[500px] mx-auto bg-white rounded-sm shadow-pricing p-12 sm:p-[60px]">
                        ...
                        <button onClick={() => googleSignIn()} className="w-full flex items-center justify-center p-3 bg-white text-body-color text-base font-medium hover:text-primary rounded-sm border border-[#DEE3F7] mb-6" fdprocessedid="b6iyuk">
                            <span className="mr-3">
